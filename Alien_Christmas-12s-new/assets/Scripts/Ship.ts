...
export default class Ship extends cc.Component {
    ...
    createBullet() {
        ...
        if (this.text.active == false) {
            ...
            if (window.matchMedia("(orientation: landscape)").matches&&this.activeBullet==false) { 
                ...
                cc.audioEngine.playEffect(this.shoot, false);
                //cc.audioEngine.playEffect(this.shoot,false);
            } else {if (window.matchMedia("(orientation: landscape)").matches&&this.activeBullet==true) {
                var Bullet = cc.instantiate(this.shipBullet2);
    ...
    endCard() {
        ...
        }else  if (window.matchMedia("(orientation:  landscape)").matches&& this.activeBullet==false) this.node.setPosition(8.808,-144.453); 
